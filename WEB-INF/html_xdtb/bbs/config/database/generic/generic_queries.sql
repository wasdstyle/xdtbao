...
# CategoryModel
# #############
CategoryModel.selectById = SELECT categories_id, title, display_order, moderated, pic_path FROM bbs_categories WHERE categories_id = ? ORDER BY title 
CategoryModel.selectAll = SELECT categories_id, title, display_order, moderated, pic_path FROM bbs_categories ORDER BY display_order
...
CategoryModel.update = UPDATE bbs_categories SET title = ?, moderated = ?, pic_path = ? WHERE categories_id = ?
CategoryModel.addNew = INSERT INTO bbs_categories (title, display_order, moderated ,pic_path) VALUES (?, ?, ?, ?)
...
UserModel.synAddUser = INSERT INTO bbs_users (username,user_active,user_password, user_email, user_regdate) VALUES (?, ?, ?, ?, ?)
...
SearchModel.getPostsDataForLucene = SELECT p.post_id, p.forum_id, p.topic_id, p.user_id, u.username, p.enable_bbcode, p.enable_smilies, p.post_time, pt.post_subject, pt.post_text, t.topic_title \
	...
	AND p.post_id = pt.post_id  \
	AND p.topic_id = t.topic_id \
	AND p.user_id = u.user_Id limit 10
	
SearchModel.getPostsTotalForLucene = SELECT COUNT(*) \
	FROM bbs_posts p, bbs_posts_text pt, bbs_users u, bbs_topics t \
	WHERE p.post_id IN (:posts:) \
	AND p.post_id = pt.post_id  \
	AND p.topic_id = t.topic_id \
