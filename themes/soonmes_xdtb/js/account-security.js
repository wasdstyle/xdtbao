		...
		.wait(function() {
			...
			$('#newpassword').focus(function(){
				...
				// 密码为纯数字或纯密码
					...
					passwordStatus = 0;
					if(conPwdVal) {
						$('#newpassword1').toolTipError(tip.pwdError);
						conPwdStatus = 0;
					}
					return false;
				// 确认密码为空
				...
				// 确认密码不为空，确认密码不等于密码
					...
					$(this).toolTipHide();
					passwordStatus = 1;
					$('#newpassword1').toolTipError(tip.twoPwdNotSame);
					conPwdStatus = 0;
				}else if(conPwdVal && conPwdVal === pwdVal){
					$(this).toolTipHide();
					passwordStatus = 1;
