$(function() {
	var remainAmount = parseFloat(($('#remain_money').val()).replace(",","")),
	minCashAmount    = parseFloat($('#min-cash-amount').text()),
	maxCashAmount    = parseFloat($('#max-cash-amount').text()),
	todayCashAmount  = parseFloat($('.today-cash-amount i').text());
	// alert(remainAmount);
	...
	$('.btn-confirm-cash').click(function() {
		if($('#cash_forbid').val()==1){
			layer.alert('您已经被禁止提现，请联系管理员！');
			return false
		}else{
			if(!$('#cash-money').val()) {
				layer.alert('请输入提现金额！');
