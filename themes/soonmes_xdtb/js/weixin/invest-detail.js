$(function() {
	/* 借款详细选项卡切换 */
	// $(".borrow-info .tab-title").changeTab({contentTab: '.borrow-info .tab-cont'});
	...
	$('#auto-input').click(function() {
		if(userMoney < minInvestAmount) {
			layer.alert('您的账户余额小于最低投标金额！<br>请充值后再投标。');
			return false;
	...
	$('#btn-invest').click(function() {
		...
		if(!$('#paypassword').length) {
			...
			layer.alert("您的可用余额小于您的投标金额，请先充值。");
			return false;
		}else {
	...
	function showTenderRecord(pageIndex){
		var option = {
			...
			url        : '/invest/detailTenderForJson.html?randID='+ escape(new Date()),
