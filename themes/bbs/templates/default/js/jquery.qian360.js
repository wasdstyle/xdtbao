			...
			if ($.browser.msie && !($.browser.version == "10.0")) { //兼容ie10以下
				...
				$(this).children("input:password").each(function() {
					var oThis = $(this),
						...
						iPlaceholder = oThis.attr("placeholder"),
						iClass = oThis.attr("class"),
						oPassWord;
					oThis.after('<input type="text" placeholder=' + iPlaceholder + ' class=' + iClass + ' />');
					oPassWord = oThis.next();
					oThis.detach();
					oPassWord.val(iPlaceholder).css("color", "#666");
		...
		pop: function(value) {
			...
			if (o.bBefore) {
				...
				function fnPop() {
					...
					if (o.iSrc) {
						var iHeight = parseInt(oPop.height()) - 40;
						oPop.find(".conts").append('<iframe src="" frameborder="0"></iframe>');
						oPop.find("iframe").css({
							height: iHeight
						});
						oPop.find("iframe").attr("src", o.iSrc);
					} else {
						oPop.find(".conts").append(o.sContent);
					}
					//是否要遮罩
					if (o.bShade) {
	...
	$.extend({
		alert: function(val, tit,fn) {
			$().pop({
				...
				iWidth: "inherit",						//宽度
				iHeight: "inherit",						//高度
				sContent: '<div class="alert_info">' + val + '</div>',		//内容
				fnClose:fn							//关闭时回调
			});
			$("#qui_pop").css({
				"width": "380px",
	...
	$.fn.extend({
		isSubmit:function(state,str){
			...
			if(!state){
				...
				.bind("click.isSubmit",function() {
					return false;
				});
			}else{
				$(this).unbind(".isSubmit").html(str).val(str);
			}
		}
	})
})(jQuery);